...
import { LyricsModel } from '#modules/songs/models/lyrics.model'
...
export class SongController implements Routes {
  ...
  public initRoutes() {
    ...
    this.controller.openapi(
      createRoute({
        method: 'get',
        path: '/songs/lyrics/{lyricsId}',
        tags: ['Songs'],
        summary: 'Retrieve lyrics by lyrics ID',
        description: 'Retrieve the full lyrics for a song using its lyrics ID.',
        operationId: 'getLyrics',
        request: {
          params: z.object({
            lyricsId: z.string().openapi({
              title: 'Lyrics ID',
              description: 'ID of the lyrics to retrieve',
              type: 'string',
              example: 'lyrics_12345'
            })
          })
        },
        responses: {
          200: {
            description: 'Successful response with lyrics',
            content: {
              'application/json': {
                schema: z.object({
                  success: z.boolean().openapi({
          ...
          400: { description: 'Bad request when lyrics ID is missing or invalid' },
          404: { description: 'Lyrics not found for the given ID' }
        }
      }),
      async (ctx) => {
        const lyricsId = ctx.req.param('lyricsId')

        const lyrics = await this.songService.getLyrics({ lyricsId })
